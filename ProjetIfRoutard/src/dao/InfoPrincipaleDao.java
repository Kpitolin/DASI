...
public class InfoPrincipaleDao {
    ...
    public static InfoPrincipale findInfoByCodeInfo(String codeInfo) {
        ...
        Query query = JpaUtil.obtenirEntityManager().createQuery("select i "
                + "from InfoPrincipale i where i.codeInfoPrincipale = :code ");
        query.setParameter("code", codeInfo);
        InfoPrincipale voyage = (InfoPrincipale) query.getSingleResult();
        if (voyage == null) {
            throw new EntityNotFoundException("Can't find informations for code"
                    + " " + codeInfo);
        }
        return voyage;
    }
