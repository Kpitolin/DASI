...
public class VoyageDao {
    ...
    public static List<Voyage> findVoyageByNomPays(String nomPays) {
        ...
        Query query = JpaUtil.obtenirEntityManager().createQuery("select v from Voyage v where v.paysDuVoyage.nom = :nom ");
    ...
    public static List<Sejour> listerSejours() {
        ...
        Query query = JpaUtil.obtenirEntityManager().createQuery("select c from Voyage c where c in (select ci from Circuit ci ) "
    ...
    public static List<Sejour> listerSejoursParPays(String nomPays) {
        ...
        Query query = JpaUtil.obtenirEntityManager().createQuery("select s from Voyage s where s in (select se from Sejour se) and s.paysDuVoyage.nom = :nom ");
        query.setParameter("nom", nomPays);
        List<Sejour> sejours = (List<Sejour>) query.getResultList();
        if (sejours == null) {
            throw new EntityNotFoundException("");
    ...
    public static Voyage findVoyageById(Integer voyageId) {
        ...
        Voyage voyage = JpaUtil.obtenirEntityManager().find(Voyage.class, voyageId);
        if (voyage == null) {
    ...
    public static Voyage findVoyageByCodeVoyage(String codeVoyage) {
        ...
        Query query = JpaUtil.obtenirEntityManager().createQuery("select v from Voyage v where v.CodeVoyage = :code ");
    ...
    public static List<InfoPrincipale> listerInfos (String CV){
        ...
        Query query = JpaUtil.obtenirEntityManager().createQuery("select v.infos from Voyage v where v.CodeVoyage =:CV");
